...
WordFrequency * get_wordfrequency_ifpresent(std::unordered_map<string,int> &frequencymap, int &numofwordfreqstosend, const char (*lookup_strings)[STRING_LENGTH], int lookupcount, int allwords=0)
 ...
 void insert_to_localmap(std::set<string> &stringset, std::unordered_map<string,std::unordered_map<string,int>> &localmap, sqlite3 *db)
 {
    char *sErrMsg =0;
           
    //************************Insertion***************************************
    //processes local map that does not have the string entry
    for(auto tword_it=stringset.begin();tword_it!=stringset.end(); tword_it++)
    {        
        for(auto cword_it = stringset.begin();cword_it != stringset.end();++cword_it)
        {            
            string insertQuery = "INSERT INTO hpctext VALUES ('"+*tword_it+"','"+*cword_it+"',1);";             
            sqlite3_exec(db, insertQuery.c_str(), NULL, NULL, &sErrMsg);
 ...
 void process_string(string &str, std::unordered_map<string,std::unordered_map<string,int>> &localmap, std::unordered_map<string, int> &frequencymap)
 {
    ...
    //**********************************************open database************************
    sqlite3 *db;    
    string dbname = "database"+to_string(myrank) +".db";
    sqlite3_open(dbname.c_str(), & db);
    cout<<"Process: "<<myrank<<" Database opened!"<<endl;
    //************************************create table**************************************
    string createQuery = "CREATE TABLE IF NOT EXISTS hpctext (tword TEXT,cword TEXT, count integer);";
    sqlite3_stmt *createStmt;
    sqlite3_prepare(db, createQuery.c_str(), createQuery.size(), &createStmt, NULL);
    if (sqlite3_step(createStmt) != SQLITE_DONE) cout << "Table creation error!" << endl;
    sqlite3_finalize(createStmt);
    cout<<"Process: "<<myrank<<" Table done!"<<endl;

    //************************BEGIN TRANSACTION***************************************
    char *sErrMsg =0;
    sqlite3_exec(db, "BEGIN TRANSACTION", NULL, NULL, &sErrMsg);
    sqlite3_free(sErrMsg);
    cout<<"Process: "<<myrank<<" Transaction begin!"<<endl;
        while(index<length)
        {
            index = str.find('\n',index+1);
            ...
            {   
                ...
                insert_to_localmap(uniquewords,localmap,db); 
                uniquewords.clear();                              
            }
            current_index=index;
        }
    ...
    sqlite3_free(sErrMsg);
cout<<"Process: "<<myrank<<" Transaction end!"<<endl;
    sqlite3_close(db);
    cout<<"Process: "<<myrank<<" Database closed!"<<endl;
 }
...
/*//////Datatype for Sending word frequencies*/
    ...
    for(std::vector<string>::iterator it = files.begin(); it != files.end(); ++it)
    {
        ...
        if(myrank ==0)
        std::cout<<"Processing completed for file:"<<(*it).c_str()<<endl;
